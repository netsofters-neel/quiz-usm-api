...
class Api extends REST_Controller {
    ...
    public function set_custom_leaderboard_post(){
        ...
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            if ($user_id && $this->post('category') && $this->post('type')) {
                ...
                $this->db->where('level', $type)->where('level', $level);
                $this->db->where('category', $category)->where('subcategory', $subcategory);
                $res = $this->db->get('tbl_custom_leaderboard')->result_array();
                if (empty($res)) {
                    $frm_data = array(
                        'user_id' => $user_id,
                        'type' => $type,
                        ...
                        'score' => ($this->post('score')) ? $this->post('score') : 0,
                        'level' => ($this->post('level')) ? $this->post('level') : 0,
                        'correct_answers' => ($this->post('correct_answers')) ? $this->post('correct_answers') : 0,
                        'question_attempted' => ($this->post('question_attempted')) ? $this->post('question_attempted') : 0,
                    );
                    $this->db->insert('tbl_custom_leaderboard', $frm_data);
                    $response['error'] = false;
    ...
    public function get_custom_leaderboard_post(){
        ...
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            $this->db->join('tbl_users u', 'u.id=m.user_id');
            $this->db->where('MONTH(m.date_created)', $month)->where('YEAR(m.date_created)', $year);
            $data_m = $this->db->get('tbl_custom_leaderboard m')->result_array();
            $total = count($data_m);
            
            $sql = "SELECT r.*, u.email,u.name,u.profile FROM ( SELECT s.*, @user_rank := @user_rank + 1 user_rank FROM ( SELECT user_id, score FROM tbl_custom_leaderboard m join tbl_users u on u.id = m.user_id WHERE m.type = $type GROUP BY user_id) s, (SELECT @user_rank := 0) init ORDER BY score DESC ) r INNER join tbl_users u on u.id = r.user_id ORDER BY r.user_rank ASC LIMIT $offset,$limit";
            // echo $sql; die();
            ...
            if (!empty($data)) {
                ...
                if ($user_id) {
                    // $user_id = $this->post('user_id');
                    $my_rank = $this->db->query("SELECT r.*, u.email,u.name,u.profile FROM ( SELECT s.*, @user_rank := @user_rank + 1 user_rank FROM ( SELECT user_id, score FROM tbl_custom_leaderboard m join tbl_users u on u.id = m.user_id WHERE m.type = $type GROUP BY user_id) s, (SELECT @user_rank := 0) init ORDER BY score DESC ) r INNER join tbl_users u on u.id = r.user_id WHERE user_id=$user_id")->result_array();
    ...
    public function get_questions_by_level_post() {
        ...
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            if ($this->post('level') && ($this->post('category') || $this->post('subcategory'))) {
                ...
                $sub_cat_id = $this->post('subcategory');
            ...
            if ($this->post('category') && $user_id) {
                ...
                if($type ==  1 || $type == '1'){
                    //$no_of_que = ' (select count(id) from tbl_question q where q.subcategory=s.id ) as no_of_que,';
                     if( $this->post('language_id')){
                    $no_of_que = ' (select count(id) from tbl_question q where q.subcategory=s.id AND q.category='.$category.' AND q.language_id= '.$language_id.') as no_of_que,';
                    // print_r($no_of_que);
                    // die();
                    }
                    else{
                ...
                if($this->post('language_id')){
                    $this->db->select('s.*, '.$no_of_que.' (select max(`level` + 0) from tbl_question q where q.subcategory=s.id  AND q.category='.$category.' AND q.language_id= '.$language_id.' ) as maxlevel');
                }else{
    ...
    public function get_categories_post() {
        ...
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            if ($this->post('type') && $user_id) {
                ...
                if($type ==  1 || $type == '1'){
                    if( $this->post('language_id')){
                    $no_of_que = ' (select count(id) from tbl_question q where q.category=c.id AND q.question_type =1 AND q.language_id= '.$language_id.') as no_of_que,';
                    }
                }
                if($type ==  2 || $type == '2'){
